...
def add_poss_mtx_nodes(node, minheap):
...
#the only thing that would change would be the if() and the calc_
        if (global_heuristic == "manhattan"):
            ...
            new_mtx_node.parent = node
            new_mtx_node.numNodes += 1
            #>>add the new node to minheap (still within if-statement)
            #add to minheap by priority(f-val)
            heapq.heappush(minheap, (new_mtx_node.f_val, new_mtx_node))
...
def a_star(init_node):
    ...
    while(bool_not_goal_state):
        ...
        #step2: within while-loop: insert new nodes
        add_poss_mtx_nodes(minheap)
        #all poss new nodes in minheap as of now

        #step3: take out of root and check with seen(set)
        #if seen, continue
        #else, add to seen
#todo: where to impl. this?: #check if goal-state: yes?-> bool = False
